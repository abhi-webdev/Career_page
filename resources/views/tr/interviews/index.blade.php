...
@section('title', 'My Assigned Technical Interviews')
@section('header_title', 'My Technical Interviews')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-0.5 text-xs text-[#6B6B6B] dark:text-[#A1A1A1]">
                Technical interviews assigned to you. Conduct system design, coding rounds, and submit scorecards.
            </p>
        </div>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 border-b border-[#E5E5E5] pb-4 dark:border-[#262626]">
        ...
        <a href="{{ route('tr.interviews.index', ['filter' => 'all']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'all' ? 'bg-blue-600 text-white shadow-xs' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
            All Assigned ({{ $metrics['total'] }})
        </a>
        <a href="{{ route('tr.interviews.index', ['filter' => 'today']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'today' ? 'bg-blue-600 text-white shadow-xs' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
            Today ({{ $metrics['today'] }})
        </a>
        <a href="{{ route('tr.interviews.index', ['filter' => 'upcoming']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'upcoming' ? 'bg-blue-600 text-white shadow-xs' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
            Upcoming ({{ $metrics['upcoming'] }})
        </a>
        <a href="{{ route('tr.interviews.index', ['filter' => 'completed']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'completed' ? 'bg-emerald-600 text-white shadow-xs' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
            Completed ({{ $metrics['completed'] }})
        </a>
        <a href="{{ route('tr.interviews.index', ['filter' => 'cancelled']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'cancelled' ? 'bg-red-600 text-white shadow-xs' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
    ...
    <div class="overflow-hidden rounded-2xl border border-[#E5E5E5] bg-white shadow-xs dark:border-[#262626] dark:bg-[#141414]">
                        ...
                        @foreach($interviews as $interview)
                            <tr class="transition hover:bg-[#F7F7F7] dark:hover:bg-[#1A1A1A]">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-1.5">
                                        @if($interview->interview_date->isToday())
                                            <span class="rounded-md bg-blue-600 px-1.5 py-0.5 text-[9px] font-bold text-white uppercase">Today</span>
                                        @endif
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="inline-flex rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase bg-blue-500/10 text-blue-600">
                                            {{ $interview->status }}
                                        </span>
                                    @endif
                                </td>
        ...
        @else
            <div class="py-16 text-center text-xs text-[#6B6B6B] dark:text-[#A1A1A1]">
                No technical interviews found in this view.
            </div>
        @endif
    </div>
