...
@section('header_title', 'Recruitment Pipeline & Screening')
...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="mt-0.5 text-xs text-[#6B6B6B] dark:text-[#A1A1A1]">
            Track candidate progression across recruitment stages.
        </p>
    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 border-b border-[#E5E5E5] pb-4 dark:border-[#262626]">
        ...
        <a href="{{ route('hr.applications.index') }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ !$curr ? 'bg-purple-600 text-white' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
            All ({{ $stageCounts['total'] }})
        </a>
        <a href="{{ route('hr.applications.index', ['status' => 'pending']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'pending' ? 'bg-amber-500 text-white' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
            Pending ({{ $stageCounts['pending'] }})
        </a>
        <a href="{{ route('hr.applications.index', ['status' => 'shortlisted']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'shortlisted' ? 'bg-blue-500 text-white' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
            Shortlisted ({{ $stageCounts['shortlisted'] }})
        </a>
        <a href="{{ route('hr.applications.index', ['status' => 'interview']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'interview' ? 'bg-brand-500 text-white' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
            Interview ({{ $stageCounts['interview'] }})
        </a>
        <a href="{{ route('hr.applications.index', ['status' => 'selected']) }}" class="rounded-xl px-3.5 py-1.5 text-xs font-bold transition {{ $curr === 'selected' ? 'bg-emerald-600 text-white' : 'border border-[#E5E5E5] bg-white text-[#6B6B6B] dark:border-[#262626] dark:bg-[#141414] dark:text-[#A1A1A1]' }}">
                    ...
                    <tbody class="divide-y divide-[#E5E5E5] font-medium dark:divide-[#262626]">
                        @foreach($applications as $app)
                            <tr class="transition hover:bg-[#F7F7F7] dark:hover:bg-[#1A1A1A]">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex rounded-full border px-2.5 py-0.5 text-[11px] font-bold capitalize bg-purple-500/10 text-purple-600 border-purple-500/20">
                                        {{ $app->status }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <a
                                        href="{{ route('hr.applications.show', $app) }}"
