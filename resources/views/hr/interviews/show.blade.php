...
@section('title', 'HR Interview: ' . $application->user->name)
@section('header_title', 'My Assigned HR Interview')
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-2 text-xs text-[#6B6B6B] dark:text-[#A1A1A1]">
        <a href="{{ route('hr.interviews.index') }}" class="hover:text-purple-600 transition">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-[#E5E5E5] pb-6 dark:border-[#262626]">
        <div>
            <div class="flex items-center gap-2">
                ...
                <span class="rounded-full bg-purple-500/10 px-2.5 py-0.5 text-[10px] font-bold text-purple-700 dark:text-purple-300 border border-purple-500/20">
    ...
    <div class="grid gap-6 lg:grid-cols-12">
        ...
        <div class="lg:col-span-7 space-y-6">
            ...
            <div class="rounded-2xl border border-[#E5E5E5] bg-white p-6 dark:border-[#262626] dark:bg-[#141414] shadow-xs">
                ...
                <div class="mt-4 grid gap-4 sm:grid-cols-2 text-xs">
                    ...
                    <div>
                        <span class="text-[#6B6B6B] dark:text-[#A1A1A1]">Company Division</span>
        ...
        {{-- Right: Interview Timeline & Evaluation Form --}}
        <div class="lg:col-span-5 space-y-6">
            ...
            <div class="rounded-2xl border border-purple-500/30 bg-white p-6 dark:border-purple-500/30 dark:bg-[#141414] shadow-xs">
                <div class="flex items-center justify-between border-b border-[#E5E5E5] pb-3 dark:border-[#262626]">
                    ...
                    <span class="rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase {{ $interview->status === 'completed' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-purple-500/10 text-purple-600' }}">
                        {{ $interview->status }}
                    </span>
                </div>
                ...
                <div class="mt-4 space-y-3 text-xs">
                    ...
                    <div class="flex justify-between">
                        <span class="text-[#6B6B6B] dark:text-[#A1A1A1]">Interviewer:</span>
                        <span class="font-bold text-purple-600 dark:text-purple-400">
                            {{ $interview->interviewer ? $interview->interviewer->name : 'You' }} (HR)
                        </span>
                    </div>

                    @if($interview->notes)
                        <div class="pt-2 border-t border-[#E5E5E5] dark:border-[#262626]">
                            <span class="text-[11px] font-bold text-[#111111] dark:text-white">Focus & Instructions:</span>
                    ...
                    @if($interview->meeting_link)
                        <div class="pt-2">
                            <a
                                ...
                                class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 py-2.5 text-xs font-bold text-white shadow-xs hover:bg-purple-700 transition"
                            >
                    ...
                    @if($interview->status === 'scheduled')
                        <div class="pt-4 border-t border-[#E5E5E5] dark:border-[#262626]">
                            <h3 class="text-xs font-bold uppercase tracking-wider text-[#111111] dark:text-white mb-2">
                                Complete Interview & Submit Decision
                            </h3>
                            <form action="{{ route('hr.applications.interview.complete', $application) }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                                ...
                                <div>
                                    <label class="block text-[11px] font-bold text-[#111111] dark:text-white mb-1">
                                        HR Recommendation <span class="text-red-500">*</span>:
                                    </label>
                                ...
                                <div>
                                    ...
                                    <textarea
                                        name="admin_feedback"
                                        rows="3"
                                        placeholder="Communication, culture fit, salary expectation, notice period..."
                                        class="w-full rounded-xl border border-[#E5E5E5] bg-white p-2.5 text-xs text-[#111111] outline-none focus:border-purple-600 dark:border-[#262626] dark:bg-[#1A1A1A] dark:text-white"
                                    ></textarea>
                                </div>
